...
const cardData = useMemo(() => {
  ...
  try {
    // 1. 下次考试预测
    const predictionItem = filteredScoreData.find(d => d.predicted !== null);
    const nextExamPrediction = {
      predicted: predictionItem?.predicted !== null ? predictionItem?.predicted?.toFixed(1) ?? "N/A" : "N/A",
      lower: predictionItem?.lower !== null ? predictionItem?.lower?.toFixed(1) ?? "N/A" : "N/A",
      upper: predictionItem?.upper !== null ? predictionItem.upper.toFixed(1) : "N/A"
    };
    ...
    if (actualScores.length >= 3) {
      ...
      const improvement = recentScores[recentScores.length - 1].actual - recentScores[0].actual;
      recentImprovement = {
    ...
    const scorePairs = filteredScoreData
    ...
    if (actualScores.length > 0) {
      const sum = actualScores.reduce((total, score) => total + score.actual, 0);
      const avg = sum / actualScores.length;
      
      const highestScore = actualScores.reduce((max, score) => 
        score.actual > max.score ? { score: score.actual, date: score.date } : max, 
        { score: -Infinity, date: "" }
      );
      
      const lowestScore = actualScores.reduce((min, score) => 
        score.actual < min.score ? { score: score.actual, date: score.date } : min, 
        { score: Infinity, date: "" }
      );
