...
const filteredClassAverageData = useMemo(() => {
  ...
  if (selectedSubject === "all") {
    ...
    data = classAverageData.filter((score) => score.subject === selectedSubject);
  }
...
const cardStats = useMemo(() => {
  ...
  if (actualScores.length >= 3) {
    ...
    improvement = recentScores[recentScores.length - 1].actual - recentScores[0].actual;
  }
  ...
  if (scorePairs.length > 0) {
    const errors = scorePairs.map(d => Math.abs(d.actual - d.predicted) / d.actual);
    accuracy = (1 - errors.reduce((sum, err) => sum + err, 0) / errors.length) * 100;
  }
                      ...
                      <div className="stats-cards">
                        <div className="stat-card primary">
                          ...
                          <p className="card-value">
                            {filteredScoreData.find((d) => d.predicted !== null)
                              ?.predicted?.toFixed(2) || "N/A"}
                          </p>
                          <p className="card-subtitle">
                            范围:{" "}
                            {filteredScoreData.find((d) => d.lower !== null)
                              ?.lower?.toFixed(2) || "N/A"}
                            -
                            {filteredScoreData.find((d) => d.upper !== null)
                              ?.upper?.toFixed(2) || "N/A"}
                          </p>
                        </div>
                       <div className="stat-card success">
  <p className="card-label">近期提升</p>
  <p className="card-value">{cardStats.improvement > 0 ? '+' : ''}{cardStats.improvement} 分</p>
