...
const Dashboard: React.FC = () => {
  ...
  const cardData = useMemo(() => {
    ...
    // 班级视图的数据计算
    if (viewMode === "class" && filteredClassAverageData && filteredClassAverageData.length > 0) {
      // 按日期排序
      const sortedData = [...filteredClassAverageData].sort(
        (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
      );
      
      // 班级平均分
      const sum = sortedData.reduce((total, data) => total + data.average, 0);
      const avg = sum / sortedData.length;
      
      // 最高与最低
      const highest = sortedData.reduce(
        (max, data) => data.average > max.value ? { value: data.average, date: data.date } : max,
        { value: -Infinity, date: "" }
      );
      
      const lowest = sortedData.reduce(
        (min, data) => data.average < min.value ? { value: data.average, date: data.date } : min,
        { value: Infinity, date: "" }
      );
      
      // 趋势计算
      let trend = { value: "稳定", change: "0.0" };
      if (sortedData.length >= 3) {
        const recentData = sortedData.slice(-3);
        const change = recentData[recentData.length - 1].average - recentData[0].average;
        
        let trendValue = "稳定";
        if (change >= 5) trendValue = "显著提升";
        else if (change > 0) trendValue = "稳步提升";
        else if (change <= -5) trendValue = "显著下降";
        else if (change < 0) trendValue = "轻微下降";
      ...
      // 模拟及格率和优秀率（实际应从API获取）
      const passRate = { value: "92.5", threshold: "60分" };
      const excellentRate = { value: "45.8", threshold: "85分" };
      
      // 班级排名（实际应从API获取）
      const classRank = { value: "3", total: "12", percentile: "前25%" };
      
      return {
        ...defaultData, // 保留学生视图数据
        
        // 班级视图数据
        classAverage: { value: avg.toFixed(1), period: "全部考试" },
        classHighest: { value: highest.value.toFixed(1), date: new Date(highest.date).toLocaleDateString() },
        classLowest: { value: lowest.value.toFixed(1), date: new Date(lowest.date).toLocaleDateString() },
        classTrend: trend,
        passRate,
        excellentRate,
        classRank
    ...
    // 年级视图的数据计算（可以类似班级视图，但按需调整）
    if (viewMode === "grade" && filteredClassAverageData && filteredClassAverageData.length > 0) {
      // 类似班级视图的计算...
      // 这里可以添加年级特有的统计数据
    }
