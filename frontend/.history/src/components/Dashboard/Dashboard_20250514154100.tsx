...
const cardData = useMemo(() => {
  ...
  try {
    ...
    const nextExamPrediction = {
      ...
      upper: predictionItem?.upper !== null ? predictionItem?.upper?.toFixed(1) ?? "N/A" : "N/A"
    };
    ...
    if (actualScores.length > 0) {
      const sum = actualScores.reduce((total, score) => total + (score.actual ?? 0), 0);
...
<div className="stat-card primary">
  ...
  <p className="card-value">{cardData.nextExamPrediction.predicted}</p>
  <p className="card-subtitle">
    范围: {cardData.nextExamPrediction.lower}-{cardData.nextExamPrediction.upper}
  </p>
</div>
...
<div className="stat-card success">
  ...
  <p className="card-value">{cardData.recentImprovement.value} 分</p>
  <p className="card-subtitle">{cardData.recentImprovement.period}</p>
</div>

// 在"预测准确度"卡片中:
<div className="stat-card info">
  <p className="card-label">预测准确度</p>
  <p className="card-value">{cardData.predictAccuracy.value}%</p>
  <p className="card-subtitle">{cardData.predictAccuracy.period}</p>
</div>
</div>
                    </div>
                  )}
