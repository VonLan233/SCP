...
// 修改 Dashboard.tsx 中的相关函数和 useEffect

// 1. 添加一个 useEffect 来监听 selectedStudent 和 viewMode 的变化
useEffect(() => {
  // 当学生或视图模式改变时，清除错误信息
  setError("");
  // 可能还需要隐藏搜索结果面板
...
// 2. 修改 handleSearch 函数，确保在开始新搜索时清除错误
const handleSearch = async (studentId: string) => {
  setLoading(true);
  setError(""); // 在开始新搜索时清除之前的错误
...
// 3. 修改 handleViewModeChange 函数，确保在切换视图模式时清除错误
const handleViewModeChange = (mode: "student" | "class" | "grade") => {
  setViewMode(mode);
  setActiveTab("prediction");
  setError(""); // 切换视图模式时清除错误
  setShowSearchResults(false); // 同时隐藏搜索结果面板
};

// 4. 修改 handleSelectSearchResult 函数，确保在选择搜索结果时清除错误
const handleSelectSearchResult = (student: Student) => {
  setSelectedStudent(student.id);
  setViewMode("student");
  setShowSearchResults(false);
  setError(""); // 选择搜索结果时清除错误
};

  // 选择搜索结果中的学生
const handleSelectSearchResult = (student: Student) => {
  console.log("手动选择搜索结果:", student); // 添加这行来调试
  
  // 设置学生ID
  setSelectedStudent(student.student_id); // 注意这里，可能需要使用student.id而不是student.student_id
