...
const Dashboard: React.FC = () => {
  ...
  const cardStats = useMemo(() => {
    ...
    const actualScores = filteredScoreData
      ...
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date));

    let improvement = 0;
    if (actualScores.length >= 3) {
      const recentScores = actualScores.slice(-3);
      improvement =
        recentScores[recentScores.length - 1].actual - recentScores[0].actual;
    }
    ...
    if (scorePairs.length > 0) {
      const errors = scorePairs.map(
        (d) => Math.abs(d.actual - d.predicted) / d.actual
      );
      accuracy =
