...
const Dashboard: React.FC = () => {
    ...
    try {
      if (viewMode === "student" && selectedStudent) {
        ...
        if (predictionResponse.success && predictionResponse.data) {
          ...
          // 确保在预测成功后总是获取模型状态
          try {
            console.log(`获取模型状态: 学生ID ${selectedStudent}, 科目 ${selectedSubject}`);
            const statusResponse = await getModelStatus(
              selectedStudent.toString(),
              selectedSubject !== "all" ? selectedSubject : "总分"
            );
  ...
  const cardStats = useMemo(() => {
    if (!filteredScoreData.length) return { improvement: 0, accuracy: 0 };
    ...
    if (actualScores.length >= 3) {
      ...
      improvement =
        (recentScores[recentScores.length - 1].actual ?? 0) - (recentScores[0].actual ?? 0);
    }

    // 计算预测准确度
    let accuracy = 94.8; // 默认值
    const scorePairs = filteredScoreData.filter(
      (d) => d.actual !== null && d.predicted !== null
    );
    if (scorePairs.length > 0) {
      const errors = scorePairs.map(
        (d) => d.predicted !== null && d.actual !== null ? Math.abs(d.actual - d.predicted) / d.actual : 0
      );
      accuracy =
        (1 - errors.reduce((sum, err) => sum + err, 0) / errors.length) * 100;
    }
                  ...
                  {activeTab === "prediction" && (
                    <div className="prediction-tab">
                      ...
                      <div className="chart-container">
                        // 在图表渲染前添加调试输出
console.log("图表使用的数据:", filteredScoreData);
console.log("是否有预测值:", filteredScoreData.some(d => d.predicted !== null));
console.log("是否有上限值:", filteredScoreData.some(d => d.upper !== null));
console.log("是否有下限值:", filteredScoreData.some(d => d.lower !== null));

// 修改折线图配置确保预测曲线显示
<ResponsiveContainer width="100%" height={300}>
  <LineChart
    data={viewMode === "student" ? filteredScoreData : filteredClassAverageData}
  >
    ...
    <Line
      type="monotone"
      dataKey={viewMode === "student" ? "actual" : "average"}
      stroke="#4f46e5"
      name={viewMode === "student" ? "实际分数" : "实际平均分"}
      strokeWidth={2}
      dot={{ r: 4 }}
    ...
    />
                      ...
                      <div className="stats-cards">
                        ...
                        <div className="stat-card success">
                          ...
                          <p className="card-value">
                            {Number(cardStats.improvement) > 0 ? "+" : ""}
                            {cardStats.improvement} 分
                          </p>
                          <p className="card-subtitle">最近3次考试</p>
                        </div>
                        <div className="stat-card info">
                          <p className="card-label">预测准确度</p>
                          <p className="card-value">{cardStats.accuracy}%</p>
                          <p className="card-subtitle">最近5次预测</p>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="history-tab">
                      ...
                      <div className="stats-cards">
                        <div className="stat-card">
                          ...
                          <p className="card-subtitle">全部考试</p>
                        </div>
                        <div className="stat-card">
                          <p className="card-label">最高分</p>
                          <p className="card-value">94</p>
                          <p className="card-subtitle">2024年3月6日</p>
                        </div>
                        <div className="stat-card">
                          <p className="card-label">最低分</p>
                          <p className="card-value">79</p>
                          <p className="card-subtitle">2023年10月3日</p>
                        </div>
                      </div>
                    </div>
                  )}
