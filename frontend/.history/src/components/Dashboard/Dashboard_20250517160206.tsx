  ...
  <div className="subject-selector">
    <select
      ...
      onChange={(e) => {
        ...
        setTimeout(() => {
          // 强制重新过滤数据
          const filteredData = newSubject === "all" 
            ? [...scoreData] 
            : scoreData.filter(score => score.subject === newSubject);
            
          // 如果需要，可以先清空数据再设置，以确保UI刷新
          setScoreData([]);
          
          // 短暂延迟后恢复数据
          setTimeout(() => {
            setScoreData(scoreData);
            console.log("已强制刷新图表数据");
          }, 50);
        }, 50);
      }}
    ...
    try {
      if (viewMode === "student" && selectedStudent) {
        ...
        if (predictionResponse.success && predictionResponse.data) {
          ...
          // 强制刷新图表 - 添加这几行
          const forceRefreshTimestamp = Date.now();
          // 使用一个临时状态变量触发重渲染
          setChartKey(forceRefreshTimestamp);
          // 格式化数值，保留两位小数
          ...
          if (useAdvancedModel) {
  // 使用高级预测API，传递科目参数
            const currentSubjectToPredict = selectedSubject !== "all" ? selectedSubject : "总分";
            console.log(`准备预测科目: ${currentSubjectToPredict}`);
            ...
            predictionResponse = await predictStudentScoresAdvanced(
              ...
              {
                ...modelSettings,
                subject: currentSubjectToPredict,
                force_retrain: true, // 强制重新训练，确保使用当前科目
              }
            );
          }
          // 确保在预测成功后总是获取模型状态
...
// 修改 tab-actions div，优化科目选择器与按钮的布局和交互 */}
                      ...
                      <div className="chart-container">
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart
                            key={`chart-${selectedSubject}-${Date.now()}`} 
                            data={
                              viewMode === "student"
                                ? filteredScoreData
