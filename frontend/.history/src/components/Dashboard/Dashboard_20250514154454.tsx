...
const Dashboard: React.FC = () => {
  ...
  const cardData = useMemo(() => {
    ...
    try {
      if (viewMode === "student" && filteredScoreData) {
        // 1. 下次考试预测
      ...
      if (
        ...
        filteredClassAverageData.length > 0
      ) {
        // 按日期排序
        ...
        const highest = sortedData.reduce(
          (max, data) =>
            data.average > max.value
              ? { value: data.average, date: data.date }
              : max,
          { value: -Infinity, date: "" }
        );
        ...
        if (sortedData.length >= 3) {
          ...
          const change =
            recentData[recentData.length - 1].average - recentData[0].average;
        ...
        return {
          ...
          classHighest: {
            ...
            date: new Date(highest.date).toLocaleDateString(),
          },
          classLowest: {
            value: lowest.value.toFixed(1),
            date: new Date(lowest.date).toLocaleDateString(),
          },
          classTrend: trend,
          passRate,
          excellentRate,
