...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedStudent && viewMode === "student") {
      ...
      .catch(err => {
        setError("加载学生数据失败: " + (err instanceof Error ? err.message : String(err)));
      });
...
const handleSelectSearchResult = async (student: Student) => {
  ...
  try {
    ...
    setError("加载学生详情失败: " + (err instanceof Error ? err.message : String(err)));
  } finally {
    setLoading(false);
  }
};
...
const analysisData = useMemo(() => {
  ...
  try {
      ...
      if (recentScores.length >= 3) {
        ...
        if (improvement > 5) {
          performances.push({
            type: "success",
            title: "强劲的提升趋势",
            description: `在最近${recentScores.length}次考试中提升了${improvement.toFixed(1)}分，表明学习习惯有效。`
          });
          suggestions.push({
            title: "继续保持当前学习方法",
            description: "已展现出良好的学习成效。"
          });
        } else if (improvement > 0) {
          performances.push({
            type: "success",
            title: "稳定的提升趋势",
            description: `在最近${recentScores.length}次考试中有${improvement.toFixed(1)}分的进步。`
          });
          suggestions.push({
            title: "适当增加学习难度",
            description: "以保持持续进步。"
          ...
          performances.push({
            type: "warning",
            title: "明显的下降趋势",
            description: `在最近${recentScores.length}次考试中下降了${Math.abs(improvement).toFixed(1)}分，需要关注。`
          });
          suggestions.push({
            title: "分析近期学习状态",
            description: "找出成绩下滑的原因。"
          });
        } else {
          performances.push({
            type: "primary",
            title: "稳定的表现",
            description: `在最近${recentScores.length}次考试中表现基本稳定。`
          });
        }
      }
    }
    ...
    // 分析预测准确度
    const scorePairs = filteredScoreData.filter(d => d.actual !== null && d.predicted !== null);
    if (scorePairs.length > 0) {
      // 计算最大误差
      const maxError = Math.max(...scorePairs.map(d => 
        d.actual !== null && d.predicted !== null ? Math.abs(d.actual - d.predicted) : 0
      ));
      ...
      if (maxError > 10) {
        performances.push({
          type: "warning",
          title: "预测与实际分数存在较大差异",
          description: `最大误差达${maxError.toFixed(1)}分，可能受外部因素影响。`
        });
        suggestions.push({
          title: "关注影响成绩波动的因素",
        ...
        performances.push({
          type: "success",
          title: "预测模型表现优秀",
          description: `预测与实际分数接近，最大误差仅${maxError.toFixed(1)}分。`
        });
      }
    }
    
    // 与班级平均的对比
    if (classAverageData && classAverageData.length > 0 && actualScores.length > 0) {
      // 找到可比较的数据点
      const comparablePoints = actualScores.filter(score => 
        classAverageData.some(classScore => 
      ...
      if (comparablePoints.length > 0) {
        const differences = comparablePoints.map(score => {
          const classScore = classAverageData.find(c => c.date === score.date);
          return classScore ? (score.actual || 0) - (classScore.average || 0) : 0;
        });
        ...
        if (avgDifference > 5) {
          ...
          performances.push({
            type: "warning",
            title: "低于班级平均",
            description: `平均落后${Math.abs(avgDifference).toFixed(1)}分，需要加强。`
          });
          suggestions.push({
            title: "考虑额外辅导或补习",
...
<div className="insight-card">
  ...
  <ul className="insight-list">
    {analysisData.performances.map((item, index) => {
      // 根据分析类型选择不同的图标
      let IconComponent;
      if (item.title.includes("下降") || item.title.includes("落后")) {
        IconComponent = <TrendingDown className={`icon ${item.type}`} />;
      } else if (item.title.includes("提升") || item.title.includes("高于")) {
        IconComponent = <TrendingUp className={`icon ${item.type}`} />;
      } else if (item.title.includes("均衡") || item.title.includes("学科")) {
        IconComponent = <Book className={`icon ${item.type}`} />;
      } else if (item.title.includes("习惯") || item.title.includes("稳定")) {
        IconComponent = <Calendar className={`icon ${item.type}`} />;
      } else {
        IconComponent = <TrendingUp className={`icon ${item.type}`} />;
      }
