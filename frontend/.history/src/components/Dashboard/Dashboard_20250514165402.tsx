...
const handleSearch = async (studentId: string) => {
  ...
  try {
    ...
    console.log("搜索响应:", response); // 添加这行来调试

    if (response.success && response.data) {
      console.log("搜索结果数据:", response.data); // 添加这行来调试
      setSearchResults(response.data);

      // 如果只有一个结果，直接选择该学生
      if (response.data.length === 1) {
        const student = response.data[0];
        console.log("自动选择搜索结果:", student); // 添加这行来调试
        
        // 设置学生ID
        setSelectedStudent(student.studeent_id); // 注意这里，可能需要使用student.id而不是student.student_id
        ...
        // 如果有多个结果，保持显示搜索结果
        console.log("找到多个搜索结果:", response.data.length);
      }
    } else {
      console.log("搜索无结果或失败:", response);
      setSearchResults([]);
      setError("未找到匹配的学生");
    }
  } catch (err) {
