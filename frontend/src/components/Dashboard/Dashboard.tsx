...
try {
  ...
  // 步骤1: 获取当前所有科目的成绩数据
  const currentScoreData = [...scoreData];
  
  if (viewMode === "student" && selectedStudent) {
    console.log(
      `开始为学生 ${selectedStudent} 的 ${currentSubject} 科目请求预测，使用${
    ...
    if (useAdvancedModel) {
      ...
      predictionResponse = await predictStudentScoresAdvanced(
        ...
        {
          ...modelSettings,
          subject: currentSubject !== "all" ? currentSubject : "总分", // 如果选择"all"，则使用"总分"作为默认科目
        }
      );
    } else {
      // 使用原有的预测API
    ...
    if (predictionResponse.success && predictionResponse.data) {
      ...
      const formattedData = predictionResponse.data.map((item) => ({
        ...item,
        predicted: item.predicted !== null ? Number(item.predicted.toFixed(2)) : null,
      ...
      // 步骤2: 使用格式化后的数据作为预测数据
      const predictionData = formattedData;
      
      // 步骤3: 创建一个合并后的数据集
      const mergedData = [...currentScoreData];
      
      // 步骤4: 合并数据
      predictionData.forEach(newScore => {
        // 尝试找到对应的现有数据（根据日期和科目匹配）
        const existingIndex = mergedData.findIndex(
          item => item.date === newScore.date && 
                  item.subject === newScore.subject &&
                  item.examId === newScore.examId
        );
        ...
        if (existingIndex >= 0) {
          ...
          mergedData[existingIndex] = {
            ...mergedData[existingIndex],
            predicted: newScore.predicted,
            lower: newScore.lower,
            upper: newScore.upper
          };
          
          console.log(`更新现有数据: ${newScore.date} - ${newScore.subject}`);
        } else {
          // 如果没有找到匹配项，添加新数据
          mergedData.push(newScore);
          console.log(`添加新数据: ${newScore.date} - ${newScore.subject}`);
        }
      });
      
      console.log("合并后数据长度:", mergedData.length);
      ...
      mergedData.forEach((score) => {
        if (score.subject) {
          allSubjects.add(score.subject);
        }
      });
      console.log("更新后可用科目:", Array.from(allSubjects));
      ...
      if (actualScores.length >= 3) {
        ...
        if (recentScores.length >= 3) {
          ...
          for (let i = 1; i < recentScores.length; i++) {
            scoreDiffs.push((recentScores[i].actual || 0) - (recentScores[i-1].actual || 0));
          }
          ...
          const variationCoef = stdDev / (recentScores.reduce((sum, score) => sum + (score.actual || 0), 0) / recentScores.length);
          ...
          // 判断趋势类型
          if (Math.abs(totalImprovement) >= 5 && rSquared >= 0.6 && variationCoef < consistencyThreshold) {
            // 强趋势 - 总体变化大，相关性高，波动小
            if (avgDiff > 0) {
              performances.push({
                type: "success",
                title: "明确的上升趋势",
                description: `在最近${recentScores.length}次考试中呈现持续上升趋势，总提升${Math.abs(totalImprovement).toFixed(1)}分，趋势一致性强。`,
              });
              suggestions.push({
                title: "继续保持当前学习方法",
              ...
              performances.push({
                type: "warning",
                title: "明确的下降趋势",
                description: `在最近${recentScores.length}次考试中呈现持续下降趋势，总下降${Math.abs(totalImprovement).toFixed(1)}分，需要重点关注。`,
              });
              suggestions.push({
                title: "全面评估学习状况",
            ...
            if (avgDiff > 0) {
              performances.push({
                type: "success",
                title: "逐步提升趋势",
                description: `在最近${recentScores.length}次考试中有${Math.abs(totalImprovement).toFixed(1)}分的整体提升，进步稳健。`,
              });
              suggestions.push({
                title: "适当增加学习挑战",
              ...
              performances.push({
                type: "warning",
                title: "轻微下降趋势",
                description: `在最近${recentScores.length}次考试中有${Math.abs(totalImprovement).toFixed(1)}分的整体下滑，需要关注。`,
              });
              suggestions.push({
                title: "针对性调整学习方法",
            ...
            performances.push({
              type: "warning",
              title: "成绩波动明显",
              description: `在最近${recentScores.length}次考试中成绩起伏较大，标准差达${stdDev.toFixed(1)}分，缺乏稳定性。`,
            });
            suggestions.push({
              title: "注重学习的稳定性和连贯性",
          ...
          if (recentScores.length >= 2) {
            const latestDiff = (recentScores[recentScores.length - 1].actual || 0) - 
            ...
            if (Math.abs(latestDiff) >= 8) {
              if (latestDiff > 0) {
                performances.push({
                  type: "success",
                  title: "最近考试大幅进步",
                  description: `最近一次考试比前一次提高了${latestDiff.toFixed(1)}分，表现优秀。`,
                });
              } else {
                performances.push({
                  type: "warning",
                  title: "最近考试明显退步",
                  description: `最近一次考试比前一次下降了${Math.abs(latestDiff).toFixed(1)}分，需要分析原因。`,
                });
                suggestions.push({
                  title: "分析最近考试退步原因",
