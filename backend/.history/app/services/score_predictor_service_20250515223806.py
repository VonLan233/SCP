    ...
    def predict_student_scores(self, student_id, steps=1, subject='总分', model_params=None, model_type=None):
        ...
        try:
            ...
            try:
                ...
                # 预测未来成绩
                # 预测未来成绩
                logger.info(f"预测学生 {student_id} 的未来 {steps} 步成绩...")
                last_exam = scores[-1]
                
                for i in range(steps):
                    # 使用预测器的专用方法
                    next_score, confidence_interval = predictor.predict_next_exam(df,subject=subject)
    ...
    def _fallback_prediction(self, scores, steps=1):
        ...
        for i in range(steps):
            ...
            if (last_exam.subject == "语文" or last_exam.subject == "数学" or last_exam.subject == "英语"):
                # 根据科目难度调整分数
                predicted_score *= self._adjust_by_difficulty(predicted_score/150, subject=last_exam.subject)
                # 生成置信区间
            ...
            # 根据科目难度调整分数
                predicted_score *= self._adjust_by_difficulty(predicted_score/100, subject=last_exam.subject)
                # 生成置信区间
                confidence_interval = random.randint(5,8) + i * 2  # 随着预测步长增加，不确定性增加
                confidence_interval = confidence_interval / 100
                lower_bound = max(0, predicted_score *(1- confidence_interval))
                ...
                # 生成置信区间
                predicted_score *= self._adjust_by_difficulty(predicted_score/750, subject=last_exam.subject)
                # 生成置信区间
                confidence_interval = random.randint(5,8) + i * 2  # 随着预测步长增加，不确定性增加
                confidence_interval = confidence_interval / 100
                logger.info(f"置信区间: {confidence_interval}")
                lower_bound = max(0, predicted_score *(1- confidence_interval))
    ...
    def train_model(self, student_id,subject, model_type=None, model_params=None):
        ...
        try:
            ...
            if not scores or len(scores) < 5:
                return {
                    'success': False,
                    'message': f"学生 {student_id} 的数据不足，至少需要5条记录，当前仅有 {len(scores) if scores else 0} 条"
                }
            ...
            # 设置模型缓存值为None，强制重新创建
            cache_key = f"student_{student_id}_{model_type or self.default_model_type}"
            if cache_key in self.model_cache:
            ...
            if isinstance(predictor, StudentScorePredictor):
                ...
                logger.info(f"开始为学生 {student_id} 训练模型，使用 {epochs} 轮...")
                ...
                if model:
                    ...
                    return {
                        'success': True,
                        'message': f"学生 {student_id} 的模型训练成功",
                        'metrics': metrics
                    }
                else:
                    return {
                        'success': False,
                        'message': f"学生 {student_id} 的模型训练失败"
                    }
            else:
                return {
                    'success': False,
                    'message': f"不支持训练 {type(predictor).__name__} 类型的模型"
                }
