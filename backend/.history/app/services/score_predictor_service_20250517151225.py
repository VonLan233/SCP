...
class ScorePredictorService:
    ...
    # 找到 __init__ 方法，更改默认模型类型
    def __init__(self, model_path='predictor/models', default_model_type='linear_regression'):
        """
        初始化预测服务
        ...
        self.default_model_type = default_model_type  # 改为线性回归
    ...
    def get_predictor_instance(self, student_id, subject='总分', model_type=None, model_params=None):
        ...
        if model_params is None:
            from predictor import DEFAULT_MODEL_PARAMS
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(student_model_path):
                    model_files = [f for f in os.listdir(student_model_path) if f.endswith('.pkl')]

                force_retrain = model_params.get('force_retrain', False) if model_params else False
                if not model_files or force_retrain:
                    logger.info(f"训练学生 {student_id} 的 {subject} 科目线性回归模型...")
                    ...
                    logger.info(f"线性回归模型训练完成，斜率: {model.coef_[0]:.2f}, 截距: {model.intercept_:.2f}")
                else:
                    logger.info(f"加载已有的线性回归模型")
                    # 加载已有模型
                ...
                if subject == '总分':
                    # 获取该学生的所有科目
                    # 修改后的替代方案:
all_subjects = {row.subject for row in Score.query.filter_by(student_id=student_id).with_entities(Score.subject).distinct()}
...
# 在 app/services/score_predictor_service.py 文件中
    ...
    def _linear_regression_predict(self, df, steps=1, subject=None):
        ...
        for i in range(steps):
            ...
            # 根据分数区间调整预测结果
            if base_prediction > mid_score + (max_score - mid_score) * 0.2:  # 高分区
                # 高分区域，进步更难
                difficulty_factor = 0.7 - 0.3 * ((base_prediction - mid_score) / ((max_score - mid_score) / 2))
                difficulty_factor = max(0.3, difficulty_factor)  # 确保不会太小
                
                # 如果趋势是上升的，减缓上升速度；如果是下降的，保持原样
                if slope > 0:
                    adjusted_slope = slope * difficulty_factor
                else:
                    adjusted_slope = slope
                
                # 计算下一步预测
                adjusted_prediction = base_prediction + adjusted_slope
                
            elif base_prediction < mid_score - (mid_score - min_score) * 0.2:  # 低分区
                # 低分区域，提升更容易
                boost_factor = 1.2 + 0.3 * ((mid_score - base_prediction) / ((mid_score - min_score) / 2))
                boost_factor = min(1.8, boost_factor)  # 确保不会太大
                
                # 如果趋势是上升的，加速上升；如果是下降的，减缓下降
                if slope > 0:
                    adjusted_slope = slope * boost_factor
                else:
                    adjusted_slope = slope * 0.8
            ...
            # 添加适当的随机波动
            noise = random.uniform(-std_error * 0.3, std_error * 0.3)
    ...
    # 在 predictor/linear_regression.py 文件中
# 修改 _apply_difficulty_adjustment 方法
    ...
    def _apply_difficulty_adjustment(self, score, subject=None):
        ...
        # 获取模型斜率
        slope = self.model.coef_[0] if hasattr(self.model, 'coef_') else 0.5
        ...
        if score > mid_score + range_score * 0.2:  # 高分区
            ...
            # 应用难度系数：减缓增长速度
            adjustment = slope * difficulty_factor if slope > 0 else slope
            ...
            # 应用提升系数：加速增长
            adjustment = slope * boost_factor if slope > 0 else slope * 0.8
            adjusted_score = score + adjustment
        else:  # 中间区域
            # 中等分数区域，保持正常增长
            adjusted_score = score + slope
