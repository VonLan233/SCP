...
class StudentScorePredictor:
    ...
    def monte_carlo_dropout_predict(self, X_test, n_samples=100):
        ...
        for _ in range(n_samples):
            # 关键修改：设置training=True启用Dropout
            pred = self.model.predict(X_test, verbose=0, training=True)
        ...
        return mean_predictions.flatten(), std_predictions.flatten(), lower_bound.flatten(), upper_bound.flatten()
    ...
    def predict_next_exam(self, student_df, exam_id=None, subject=None):
        ...
        # mean_pred, std_pred, lower_bound, upper_bound = self.monte_carlo_dropout_predict(latest_sequence)
        print("MC Dropout原始预测:", mean_pred, std_pred)
        print("MC Dropout置信区间:", lower_bound, upper_bound)
        ...
        print("反转标准化后:", mean_pred_original, lower_bound_original, upper_bound_original)
