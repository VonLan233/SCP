...
class LinearRegressionPredictor:
    ...
    def _save_model(self):
        ...
        try:
            ...
            # 保存线性回归模型参数
            model_info = {
                "coef": float(self.model.coef_[0]) if hasattr(self.model, 'coef_') else 0,
                "intercept": float(self.model.intercept_) if hasattr(self.model, 'intercept_') else 0,
                "timestamp": datetime.now().isoformat()
            }
            
            with open(os.path.join(self.model_path, 'linear_model_params.json'), 'w') as f:
            ...
            self.is_trained = True
            logger.info(f"线性回归模型保存到: {self.model_path}")
            return True
        except Exception as e:
    ...
    def _load_model(self):
        ...
        try:
            ...
            params_file = os.path.join(self.model_path, 'linear_model_params.json')
            ...
            # 退回到加载参数
                ...
                self.is_trained = True
                logger.info(f"已从参数加载线性回归模型")
                return True
            else:
                logger.warning(f"未找到模型文件: {model_file} 或参数文件: {params_file}")
                return False
